#https://mcfromnz.wordpress.com/2011/10/02/latent-class-mixture-modeling-with-graphics/
# load libraries
library(ggplot2)
library(catspec)
library(gridExtra)
...
dat <- read.csv("https://raw.githubusercontent.com/nzcoops/blog_code/master/lcmmBlogData.csv",
                stringsAsFactors = F)
...
###
# 2 Groups
dummy$id<-as.numeric(dummy$id)
d2 <- lcmm(y ~ x,
           random = ~x,
           subject = 'id',
           mixture = ~x,
           ng = 2, idiag = TRUE, data = dummy, link="linear")
summary(d2)
postprob(d2)
...
###
# 3 Groups
d3 <- lcmm(y ~ x, random = ~x, subject = 'id',
           mixture = ~x,
           ng = 3, idiag = TRUE, data = dummy, link = "linear")
summary(d3)
postprob(d3)
